# My Project
This repository consists of the blt-drupal-example.
...
```
$ composer create-project --no-interaction acquia/drupal-recommended-project blt-drupal-example
```
...
```
$ git clone git@github.com:<account>/blt-drupal-example.git
```
6. To ensure that upstream changes to the parent repository may be tracked, add the upstream locally as well.
```
$ git remote add upstream git@github.com:acquia-pso/blt-drupal-example.git
```
...
```
$ lando blt setup --site=blt-drupal-example
```
...
$ git status
$ git add -p
$ git commit -m"blt-drupal-example-000: Committing new changes to site."
$ git push --set-upstream origin blt-drupal-example-000-new-site-change
```
...
* Local site URL: http://blt-drupal-example.lndo.site
